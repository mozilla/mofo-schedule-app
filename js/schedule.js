function Schedule(options) {
    ...
    schedule.init = function(options) {
        ...
        schedule.tabList = [
            { name: 'Friday', displayName: 'Fri', tabDate: new Date(2016,0,22) },
            { name: 'Saturday', displayName: 'Sat', tabDate: new Date(2016,0,23) },
            //{ name: 'Sunday', displayName: 'Sun', tabDate: new Date(2016,10,8) },
    ...
    schedule.addListControls = function() {
        ...
        $('#list-filter').change(function() {
            ...
            if ($('.session-list-item:visible').length == 0) {
                $('#no-results').remove();
                $('#list-filter').after('<p id="no-results">No matching results found.</p>');
                $('#list-filter').addClass("no-results");
            } else {
                $('#no-results').remove();
                $('#list-filter').removeClass("no-results");
            }
            return false;
        }).keyup(function() {
            $(this).change();
    ...
    schedule.showFavorites = function() {
        // provide some user instructions at top of page
        schedule.$container.empty().append('<p class="overline">Favorited sessions are stored on your device.</p>').append(schedule.sessionListTemplate);
